...
import static frc.robot.subsystems.Northstar.NorthstarConstants.*;
...
import frc.robot.subsystems.CommandSwerveDrivetrain.VisionObservation;
import frc.robot.util.Northstar.LoggedTracer;
import frc.robot.util.Northstar.VirtualSubsystem;
...
public class VisionNorthstar extends VirtualSubsystem {
  private final Supplier<AprilTagLayoutType> aprilTagLayoutSupplier;
  private final NorthstarInterface[] io;
  ...
  public void periodic() {
    ...
    for (int i = 0; i < io.length; i++) {
      ...
      boolean disconnected = disconnectedTimers[i].hasElapsed(disconnectedTimeout) || !inputs[i].ntConnected;
      if (disconnected) {
        disconnectedAlerts[i].setText(inputs[i].ntConnected
    ...
    for (int instanceIndex = 0; instanceIndex < io.length; instanceIndex++) {
      // Loop over frames
      for (int frameIndex = 0; frameIndex < aprilTagInputs[instanceIndex].timestamps.length; frameIndex++) {
        ...
        switch ((int) values[0]) {
          ...
          case 2:
            ...
            if (error0 < error1 * ambiguityThreshold || error1 < error0 * ambiguityThreshold) {
              Rotation2d currentRotation = drivetrain.get().getState().Pose.getRotation();
    ...
    allVisionObservations.stream()
        ...
        .forEach(drivetrain.get()::addVisionMeasurement);
